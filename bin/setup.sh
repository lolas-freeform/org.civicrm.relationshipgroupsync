...
# append Group Sync schema to core schema
sed -i 's#</database>##' "$CIVIROOT/xml/schema/Schema.xml"
grep "<xi:include" "$RGSROOT/xml/schema/Schema.xml" >> "$CIVIROOT/xml/schema/Schema.xml"
echo "</database>" >> "$CIVIROOT/xml/schema/Schema.xml"

if [ ! -e "$CIVIROOT/xml/schema/Relationshipgroupsync" ] ; then
  ln -s $RGSROOT/xml/schema/Relationshipgroupsync $CIVIROOT/xml/schema/Relationshipgroupsync
fi
cd $CIVIROOT/xml
php GenCode.php
# (There may be extra arguments to pass into GenCode.php; not sure)

cp -f $CIVIROOT/CRM/Relationshipgroupsync/DAO/* $RGSROOT/CRM/Relationshipgroupsync/DAO/
mv $CIVIROOT/xml/schema/Schema.xml.backup $CIVIROOT/xml/schema/Schema.xml

unlink $CIVIROOT/xml/schema/Relationshipgroupsync
rm -rf $CIVIROOT/Relationshipgroupsync
